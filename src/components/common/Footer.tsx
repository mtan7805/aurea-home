...
const Footer = () => {
  return (
    <div
      id="footer"
      className="w-full px-5 md:px-[50px] lg:px-[130px] py-12 bg-[#1c120c] border-t border-t-[#9a542c] flex flex-col gap-10 lg:flex-row lg:items-start lg:justify-between items-center justify-center"
    >
      {/* Cột trái */}
      <div className="flex lg:w-[32%] w-full flex-col gap-3">
        ...
        {contactList.map((item) => {
          ...
          return (
            <div
              key={item.id}
              className="flex items-center gap-[10px] text-[#efefef]"
            >
      ...
      <div className="w-full lg:w-2/3 flex sm:flex-row flex-col items-start justify-between gap-8">
        {footerSections.map((section) => (
          <div
            key={section.id}
            className="flex md:w-[50%] w-full flex-col gap-[12px]"
          >
